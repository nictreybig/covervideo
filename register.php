...
{ // Handle the form.
    ...
    if (strlen($trimmed['tree_key']) >= 1) 
    {
    $key = password_hash($trimmed['tree_key'], PASSWORD_DEFAULT);
    }
    else 
    {
		echo '<p class="error">By launching this site, I confirm that there is<br> no key-entry required for view of this covervideo</p>';
	}
		...
		if (mysqli_num_rows($r) == 0) 
		{
		        ...
		        $q = "INSERT INTO users (link_name, pass, tree_key) VALUES ('$name', '$p', '$key')";
			    $r = mysqli_query($dbc, $q);

			    if (mysqli_affected_rows($dbc) == 1) 
			    {
				    echo '<h3>Tap the Space Shuttle to activate<br><br><u> covervideo.com/' . $name . '</u></h3>';
				    ?><br>&emsp;&emsp;&emsp;&emsp;<a href ="login.php"><i class='fas fa-space-shuttle fa-rotate-270' id="emailhover" style="font-size:144px;"></i></a>
				    <?php
				        echo '<h3>Regards,</h3>';
...
function treekey(){
document.getElementById('box2').placeholder = "Share this 1-10 Character Key with Public Visitors";
}
function treekeyreturn(){
document.getElementById('box2').placeholder = "Key";
}
function emailed(){
...
#notchspin
{
    ...
    <h1><b>&nbsp;<i class='fas fa-circle-notch fa-rotate-90' title='Auto Search' id='whiteoutgreenpeace' style='font-size:48px;'></i>reate Key-Protected</b> <br><br>&nbsp;covervideo.com/<span id="username"></span> </h1><br><br>
    &nbsp;<input type="texttreeid" autocapitalize="none" name="link_name" spellcheck="false" placeholder="ID" style="font-family:FontAwesome;" id="box1" onfocus="treeid()" onblur="treeidreturn()" size="40" maxlength="60" value="<?php if (isset($trimmed['link_name'])) echo $trimmed['link_name'];?>" oninput="sayHi();" autofocus="autofocus">&nbsp;&nbsp; </div><br>&nbsp;&nbsp; <button type="button" class="noclick" type="one" tabindex="-1" style="width: 50%; font-family:FontAwesome; background-color:#1E6C89;border:6px solid #74e2f3; box-shadow:1px 1px #1E6C89;"onclick="copyToClipboard('#username')" onmouseover="copyToClipboard('#username')">Tap Here to Copy Login ID</button><br><br><h2>&nbsp;&nbsp; <i class='fas fa-circle-notch fa-rotate-90' title='Auto Search' id='whiteoutgreenpeace' style='font-size:34px;'></i>reate Login Password <br>&nbsp;&nbsp; (10+ Characters Long)</h2><br><br>&nbsp;&nbsp; <input type="password" id ="box3" placeholder="Login Password"name="password1" autocapitalize="none" autocomplete="new-password" size="43" value="<?php if (isset($trimmed['password1'])) echo $trimmed['password1']; ?>"><br><br>&nbsp;&nbsp; <button type="button" id="viewpassword" class="noclick" tabindex="-1" onclick="showit()" onmouseover="showit()"><i class="fa fa-eye fa-rotate-180"></i><i class="fa fa-eye fa-rotate-180"></i> Password</button> <br><br>&nbsp;&nbsp; <input type="password" placeholder="Confirm Password" autocapitalize="none" id="box4" name="password2" autocomplete="new-password" size="20" value="<?php if (isset($trimmed['password2'])) echo $trimmed['password2']; ?>">
	  <br><br><br><br><br><h2>&nbsp;&nbsp; <i class='fas fa-circle-notch fa-rotate-90' id='whiteoutgreenpeace' style='font-size:34px;'></i>reate Public Key, <br>&nbsp;&nbsp; Required for Visitors <br>&nbsp;&nbsp; To Play My CoverVideo</h2><br><br> &nbsp;&nbsp;<input type="passwordemail" id="box2" placeholder="Key" autocapitalize="none" onblur="treekeyreturn()" autocomplete="new-password" spellcheck="false" name="tree_key" size="32" onfocus="treekey()" onblur="treekey" maxlength="60" value="<?php if (isset($trimmed['tree_key'])) echo $trimmed['tree_key']; ?>" style="margin-left: 5px; font-family:Arial, FontAwesome" ><br><br>&nbsp;&nbsp; <button type="button" id="peekkeyoutline" class="noclick" tabindex="-1" onclick="showcvkey()" onmouseover="showcvkey()"> <i class="fa fa-eye fa-rotate-180"></i><i class="fa fa-eye fa-rotate-180"></i> Key</button><br><br>&nbsp;&nbsp;&nbsp;&nbsp;<br><br><br><br>
    <script>
        var counter = 0;
